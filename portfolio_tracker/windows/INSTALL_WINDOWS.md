...
|Datamap|`/share/portfolio\\\_tracker`|`%LOCALAPPDATA%\\\\PortfolioTracker\\\\data`|
...
**1. Python installeren.** Versie **3.11 of 3.12** van [https://www.python.org/downloads/](https://www.python.org/downloads/).
...
`\\\\share\\\\portfolio\\\_tracker` op je Home Assistant naar de datamap die `setup.bat`
toonde. Doe dat *voor* je de app de eerste keer start. De migraties in `init\\\_db()`
...
```bat
@echo off
set "DATA\\\_DIR=D:\\\\Data\\\\PortfolioTracker"
set "BIND\\\_ADDRESS=0.0.0.0"
```
...
Zet je `BIND\\\_ADDRESS` op `0.0.0.0`, dan is de app ook bereikbaar vanaf je telefoon
...
of zet `APP\\\_PORT` op iets anders in `config.local.bat`.
